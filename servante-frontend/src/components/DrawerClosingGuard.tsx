...
const CAPTURE_W = 640;
const CAPTURE_H = 360;
const HAND_CONFIRM_FRAMES = 2;  // consecutive frames to confirm hand entered
const CLEAR_CONFIRM_FRAMES = 4; // consecutive frames without hand to confirm it left
const GUARD_SECONDS = 15;       // total monitoring window after close command

const sleep = (ms: number) => new Promise<void>(r => setTimeout(r, ms));

type GuardPhase = 'monitoring' | 'hand-detected' | 'alert' | 'safe';

// Minimal class→display name map (same model classes)
...
const DrawerClosingGuard: React.FC<DrawerClosingGuardProps> = ({ drawerId, onComplete }) => {
  ...
  // Keep phaseRef in sync; pause timer during alert (user must return tool first)
  useEffect(() => {
  ...
  // Command drawer to close on mount
  useEffect(() => {
      ...
      <div className={`max-w-2xl w-full bg-white rounded-3xl shadow-2xl p-8 border-2 ${borderColor} transition-colors duration-500`}>
        ...
        <div className="flex flex-col items-center gap-2 mb-5">
          ...
          <h2 className="text-2xl font-bold text-slate-900">
            ...
            : phase === 'alert'        ? 'Outil non autorisé détecté'
          ...
          <p className="text-sm text-slate-500 text-center">
            {phase === 'safe'
              ...
              : phase === 'alert'
              ? 'Un outil a été retiré sans autorisation lors de la fermeture.'
              : phase === 'hand-detected'
              ? 'Retirez votre main du tiroir pour continuer la fermeture.'
        ...
        {phase === 'alert' && (
          <div className="mb-4 rounded-xl border-2 border-red-400 bg-red-50 px-5 py-4">
            <p className="text-xs font-bold uppercase tracking-wide text-red-600 mb-2">
              Outil(s) pris lors de la fermeture
            </p>
        ...
        {phase === 'hand-detected' && (
          <div className="mb-4 rounded-xl border-2 border-amber-400 bg-amber-50 px-5 py-3 flex items-center gap-3 animate-pulse">
            ...
            <p className="text-sm font-bold text-amber-800">
              Moteur arrêté — Retirez votre main pour que la fermeture reprenne.
            </p>
          </div>
        )}
        ...
        {phase !== 'safe' && (
          <div className={`relative bg-black rounded-2xl overflow-hidden border-4 shadow-lg mb-4 transition-colors duration-300 ${borderColor}`}
            ...
            {/* Timer badge — only during monitoring */}
            {phase === 'monitoring' && (
